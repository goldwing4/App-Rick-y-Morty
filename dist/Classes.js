...
export class Character {
    ...
    location;
    episode;
    constructor(id, name, status, species, gender, image, origin, location, episode) {
        this.id = id;
        this.name = name;
        this.status = status;
    ...
    async getModalContent() {
        ...
        if (this.episode && this.episode.length > 0) {
            try {
                ...
                const actualEpisodes = Array.isArray(episodesData)
                    ? episodesData
                    : [episodesData];
                const episodeNames = actualEpisodes.map((ep) => `${ep.episode} - ${ep.name}`); // Formato S01E01 - Nombre
                episodesHtml = `<p><strong>Aparece en:</strong></p><ul>${episodeNames
                    .map((name) => `<li>${name}</li>`)
                    .join("")}</ul>`;
            }
            catch (error) {
                console.error(`Error al obtener episodios para el personaje ${this.name}:`, error);
      ...
      <div class="modal-content-character">
        <img src="${this.image}" alt="${this.name}" class="modal-character-image-large">
        <h3>${this.name}</h3>
    ...
    async getModalContent() {
        ...
        if (this.characters && this.characters.length > 0) {
          ...
          `)
                ...
                console.error(`Error al obtener personajes para el episodio ${this.name}:`, error);
...
export class ApiService {
    ...
    async getByIds(type, idsArray) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Error HTTP: ${response.status} al obtener ${type}s por IDs.`);
            }
            const data = await response.json();
            const itemsData = Array.isArray(data) ? data : [data];
            const instances = itemsData.map((itemData) => {
                if (type === "character") {
                    return new ClassConstructor(itemData.id, itemData.name, itemData.status, itemData.species, itemData.gender, itemData.image, itemData.origin, itemData.location, itemData.episode);
                }
                else {
                    return new Episode(itemData.id, itemData.name, itemData.air_date, itemData.episode, itemData.characters);
                }
            });
            return instances;
        }
    ...
    async #getAllCharacters(url, params) {
        try {
            ...
            const characters = charactersData.results.map((charData) => new Character(charData.id, charData.name, charData.status, charData.species, charData.gender, charData.image, charData.origin, charData.location, charData.episode));
    ...
    async #getAllEpisodes(url, params) {
        try {
            ...
            const episodes = episodesData.results.map((episodeData) => new Episode(episodeData.id, episodeData.name, episodeData.air_date, episodeData.episode, episodeData.characters));
